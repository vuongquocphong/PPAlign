import numpy as np
from ppalign import model
from ppalign.utils import *
...
class Bertalign:
    def __init__(self,
        ...
        if src_lang != 'zh' or tgt_lang != 'vi':
            raise ValueError("Source language must be Chinese (zh) and target language must be Vietnamese (vi).")
    ...
    def align_sents(self):
        ...
        second_pointers = second_pass_align(self.src_vecs, self.tgt_vecs, self.src_lens, self.tgt_lens,
                                            ...
                                            self.char_ratio, margin=self.margin, len_penalty=self.len_penalty, sentence_num_penalty=self.sentence_num_penalty, union_score=self.union_score)
    ...
    def _get_line(bead, lines, join_char):
        line = ''
        if len(bead) > 0: line = join_char.join( lines[ bead[0] : bead[-1] + 1 ] )
        return line